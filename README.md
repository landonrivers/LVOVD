...
**Add a suffix**, beside **Process File**, controls the reviewed download name (default ` - processed`; uncheck for no suffix). It also names byte-identical no-op downloads without changing or copying source bytes. A new suffix creates a new draft and never renames an earlier completed result.
