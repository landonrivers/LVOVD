...
### Implemented Roadmap 7B boundary
...
Convert always names its input as the original source. Cuts are preserved but not applied to conversion. The server accepts only the current owning original asset ID, recomputes the reviewed deterministic plan key at Start, and requires acknowledgement of known additional track/artwork/chapter omissions. Paths, mappings, filters, and encoder names are not client inputs. Edited-result handoff remains deferred.
...
One conversion slot exists across the Node process, with one expensive operation per workspace and no batch queue. This does not serialize existing Download/Edit work in other workspaces. Cancel acknowledges within a bounded interval; the slot stays reserved until process exit is observed. Failed/cancelled reruns preserve previous successful outputs. New attempts use unique files, validate before publication, then retire only the previous converted result. Retained partial/retired-file cleanup ownership blocks accumulating more conversion attempts until explicit cleanup succeeds. Discard immediately invalidates IDs and closes owned readers; physical cleanup and retry remain truthful if a process or file lock delays removal.

Inputs and converted outputs each have a 100 GiB cap, with active and final output enforcement. A tool size limit cannot publish a truncated success. Disk use can simultaneously include the source, playback proxy, edited result, previous conversion, and new attempt; free space is not guaranteed. ENOSPC is a local failure. These temporary operations do not create Download jobs or History entries. Later targets (including Opus/FLAC/WAV), edited-result handoff, batches, and images remain deferred.
