...
Process Selected File submits the selected reviewed draft. Its ready heading reads “Ready to process selected file”; submitted/result revisions remain available when relevant. Review All Files shows the total current file count in parentheses and builds an explicit per-file review, with omission acknowledgements and selectable eligible entries. **Queue And Process** admits those reviewed files. Missing/unsupported sources are explained and excluded; files already queued/running or already successfully processed at the same revision are excluded from batch selection. An unchanged eligible source uses the complete original bytes. There is no automatic processing, downloading, or retry when settings change or a job fails.

One **Processing results** area presents single-file and batch outcomes. Successful admission brings it into view once; progress updates preserve focus and do not scroll or change the selected editor. Rows lead with the validated output's format and actual filename, followed by a **Download MP3** (or other actual format) action, secondary Source information, explicit **Edit source** navigation, and expandable result details. Counts distinguish completed, processing, queued, failed/cancelled attempts, and available downloads. Effective stream actions are retained with the immutable processing snapshot so an encoded MP3 is not described as unchanged audio merely because no override was requested. Previous successful downloads remain clearly marked after failed/cancelled replacements, including repeats of the same draft; they are not counted as successful outputs of that latest attempt.

Results use compact rows in a bounded, keyboard-scrollable list. Source and detailed encoding facts stay expandable. **Download All (N)** requests the existing output files, with their original result filenames and formats, once all listed latest attempts have succeeded. It stays unavailable while any listed attempt is pending, failed, or cancelled, so a previous result cannot silently stand in for that attempt. Genuine unchanged-byte results remain eligible. Browser requests are paced over the fixed result snapshot; removal, replacement, or new processing of a captured file stops the remaining requests instead of silently substituting results. Discard clears pending requests. This is an explicit browser download action, not a ZIP, another processing queue, a media copy, or a provider request. The browser may ask to allow multiple downloads; LVOVD reports that downloads were requested rather than claiming all files were saved. Individual Download actions remain available.
